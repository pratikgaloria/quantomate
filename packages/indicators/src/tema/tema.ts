...
interface IIndicatorParamsTEMA<T> {
  attribute?: T extends object ? keyof T : string;
  period?: number;
}

export class TEMA<T = number> extends Indicator<IIndicatorParamsTEMA<T>, T> {
  constructor(name = 'TEMA', params: IIndicatorParamsTEMA<T>) {
    super(
      name,
      function (this: TEMA<T>, dataset: Dataset<T>) {
        ...
        const lastEMA3 = dataset.at(-1)?.getIndicator(ema3Name);

        if (lastEMA1 !== undefined && !isNaN(lastEMA1) &&
            lastEMA2 !== undefined && !isNaN(lastEMA2) &&
            lastEMA3 !== undefined && !isNaN(lastEMA3)) {
        ...
        const ema2Values: number[] = new Array(datasetLength).fill(NaN);
        ema2Values[2 * period - 2] = ema2;
        ...
        // Calculate EMA3
        if (datasetLength < 3 * period - 2) {
             return NaN;
        }

        sum = 0;
        for (let i = 0; i < period; i++) {
             sum += ema2Values[2 * period - 2 + i];
        }
        ema3 = sum / period;

        for (let i = 3 * period - 2; i < datasetLength; i++) {
             const val = ema2Values[i];
             ema3 = val * smoothing + ema3 * (1 - smoothing);
        }
      ...
      },
      {
        ...
        beforeCalculate: (dataset: Dataset<T>) => {
             ...
             const startIndexEMA3 = 3 * period - 3;
             ...
             for (let i = 0; i < dataset.length; i++) {
                  ...
                  if (!isNaN(currentEMA2)) {
                      ...
                      // Update storage immediately so subsequent calculations can see it if needed
                      // Although here for EMA3 we can just use currentEMA2
                  ...
                  if (!isNaN(currentEMA2)) {
                      if (isNaN(ema3Prev)) {
                           if (i >= startIndexEMA3) {
                               let sum = 0;
                               // Use currentEMA2 for j=0
                               sum += currentEMA2;
                               for (let j = 1; j < period; j++) {
                                    sum += dataset.at(i - j)?.getIndicator(ema2Name) || 0;
                               }
                               ema3Prev = sum / period;
                               currentEMA3 = ema3Prev;
                           }
                      } else {
                           ema3Prev = currentEMA2 * smoothing + ema3Prev * (1 - smoothing);
                           currentEMA3 = ema3Prev;
                      }
                  }

                  if (!isNaN(currentEMA3)) {
                      quote.setIndicator(ema3Name, currentEMA3);
                      dataset.mutateAt(i, quote);
                  }
             }
        }
    ...
    this.withIncremental((prevTEMA: number, newQuote: Quote<T>, dataset: Dataset<T>) => {
        const { attribute, period = 9 } = params;
        const smoothing = 2 / (period + 1);
        ...
        const prevEMA3 = getPrevIndicator(ema3Name);
        ...
        if (isNaN(newEMA2)) {
             ...
             if (isNaN(prevEMA3)) {
                 if (dataset.length === 3 * period - 2) {
                      let sum = 0;
                      sum += newEMA2;
                      for (let i = 1; i < period; i++) {
                           sum += dataset.at(dataset.length - 1 - i)?.getIndicator(ema2Name) || 0;
                      }
                      newEMA3 = sum / period;
                 } else {
                      newEMA3 = NaN;
                 ...
                 newEMA3 = newEMA2 * smoothing + prevEMA3 * (1 - smoothing);
             }
        }

        if (!isNaN(newEMA3)) {
             const currentQuote = dataset.at(-1)!;
             currentQuote.setIndicator(ema3Name, newEMA3);
             dataset.mutateAt(dataset.length - 1, currentQuote);
             return 3 * newEMA1 - 3 * newEMA2 + newEMA3;
        }
