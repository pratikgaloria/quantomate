...
async function run() {
    ...
    const strategy = new StrongPullback({
        ...
        rvolPeriod: 20,
        atrPeriod: 14,
        rvolThreshold: 1.1, // Lowered slightly to match data variance
        vwapDistanceAtrRatio: 0.5,
        atrMultiplier: 1.0,
        tp1_R: 1.0,
        tp2_R: 2.0,
        tp1_pct: 100 // Fully exit at TP1 to match TV CSV sample "L-TP1"
    });
    ...
    const report = backtest.run({
        config: { 
            capital: 100000, 
            commission: 0.009 // Estimated absolute cost per share for NVDA
        },
