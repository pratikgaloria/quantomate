...
async function run() {
    ...
    const strategy = new StrongPullback({
        ...
        rvolPeriod: 20,
        atrPeriod: 14,
        rvolThreshold: 1.0, // Reduced slightly for validation
        vwapDistanceAtrRatio: 1.0 // Increased slightly for validation
    });
    ...
    report.trades.forEach(t => {
        console.log(`${t.type.toUpperCase()} | ${new Date(t.quote.timestamp).toISOString()} | Price: ${t.tradedValue.toFixed(2)} | ${t.exitReason || '-'}`);
    });
}
