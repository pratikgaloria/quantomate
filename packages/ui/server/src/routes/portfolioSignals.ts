...
const ACTIVE_STRATEGIES = [
  { id: 'golden-cross', params: { fastPeriod: 50, slowPeriod: 200, direction: 'long' } },
  { id: 'rsi-mean-reversion', params: { rsiPeriod: 14, oversoldThreshold: 30, overboughtThreshold: 70, useTrendFilter: false, direction: 'long' } },
  { id: 'bollinger-bands', params: { period: 20, multiplier: 2, direction: 'long' } },
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`Refreshing fundamentals (cachedOnly=${isCached}) for: ${symbolsList.join(', ')}`);
    const fundamentalsList = await FundamentalService.refreshSignals(fundamentalsInput, undefined, isCached);
    ...
    for (const symbol of symbolsList) {
      try {
        ...
        for (const stratInfo of ACTIVE_STRATEGIES) {
          ...
          let latestSignalValue = 'HOLD';
          let latestSignalPrice = stockData[stockData.length - 1].close;
          let latestSignalDate = stockData[stockData.length - 1].date;
          ...
          for (let i = dataset.length - 1; i >= 0; i--) {
            ...
            if (strategyValue) {
              ...
              if (posVal === 'entry') {
                latestSignalValue = strategyValue.position.options?.short ? 'SHORT' : 'BUY';
                latestSignalPrice = stratInfo.id === 'pivot-trend' ? val.open : val.close;
                latestSignalDate = val.date;
                break;
              } else if (posVal === 'exit') {
                latestSignalValue = strategyValue.position.options?.short ? 'COVER' : 'SELL';
                latestSignalPrice = stratInfo.id === 'pivot-trend' ? val.open : val.close;
                latestSignalDate = val.date;
                break;
              }
    ...
    res.status(500).json({ error: 'Failed to fetch portfolio signals', message: error.message });
  }
});
...
router.get('/chart/:symbol', async (req: Request, res: Response) => {
  try {
    ...
    console.error(`Error loading chart details for ${req.params.symbol}:`, error);
    res.status(500).json({ error: 'Failed to fetch chart details', message: error.message });
  }
});
