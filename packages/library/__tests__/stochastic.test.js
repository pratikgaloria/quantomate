...
describe('Stochastic Oscillator should return the correct value', () => {
    ...
    describe('%K (Fast Stochastic)', () => {
        const name = 'stoch_k';
        const kPeriod = 14;
        const stochastic = new src_1.Stochastic(name, {
            kPeriod,
            type: 'k',
            high: 'high',
            low: 'low',
        ...
        it('When dataset length is less than kPeriod.', () => {
            const result = stochastic.calculate(new core_1.Dataset(data.slice(0, kPeriod - 1)));
            expect(isNaN(result)).toBe(true);
        });
        it('When dataset length is equal to kPeriod.', () => {
            const result = stochastic.calculate(new core_1.Dataset(data.slice(0, kPeriod)));
    ...
    describe('%D (Slow Stochastic)', () => {
        ...
        it('When dataset length is less than kPeriod + dPeriod - 1.', () => {
            ...
            const result = stochastic.calculate(new core_1.Dataset(data.slice(0, minRequired - 1)));
            expect(isNaN(result)).toBe(true);
        });
    ...
    describe('Value range validation', () => {
        it('%K should always be between 0 and 100.', () => {
            const ds = new core_1.Dataset(data);
            const stochastic = new src_1.Stochastic('stoch_k', {
                kPeriod: 14,
                type: 'k',
                high: 'high',
                low: 'low',
    ...
    describe('Edge cases', () => {
        it('Should handle zero range (high === low).', () => {
            ...
            const ds = new core_1.Dataset(flatData);
            const stochastic = new src_1.Stochastic('stoch_k', {
                kPeriod: 14,
                type: 'k',
                high: 'high',
    ...
    describe('Custom parameters', () => {
        it('Should work with custom kPeriod.', () => {
            const ds = new core_1.Dataset(data);
            const stochastic = new src_1.Stochastic('stoch_custom', {
                kPeriod: 10,
                type: 'k',
                high: 'high',
                low: 'low',
        ...
        it('Should work with custom dPeriod.', () => {
            const ds = new core_1.Dataset(data);
            const stochastic = new src_1.Stochastic('stoch_d_custom', {
                kPeriod: 14,
                dPeriod: 5,
                type: 'd',
                high: 'high',
