...
describe('Williams %R should return the correct value', () => {
    ...
    describe('Basic Williams %R calculation', () => {
        ...
        const period = 14;
        const williamsR = new src_1.WilliamsR(name, {
            period,
            high: 'high',
            low: 'low',
            close: 'close',
        ...
        it('When dataset length is less than period.', () => {
            const result = williamsR.calculate(new core_1.Dataset(data.slice(0, period - 1)));
            expect(isNaN(result)).toBe(true);
        });
        it('When dataset length is equal to period.', () => {
            const result = williamsR.calculate(new core_1.Dataset(data.slice(0, period)));
    ...
    describe('Value range validation', () => {
        it('%R should always be between -100 and 0.', () => {
            const ds = new core_1.Dataset(data);
            const williamsR = new src_1.WilliamsR('williams_r', {
                period: 14,
                high: 'high',
                low: 'low',
                close: 'close',
    ...
    describe('Custom parameters', () => {
        it('Should work with custom period.', () => {
            const ds = new core_1.Dataset(data);
            const williamsR = new src_1.WilliamsR('williams_r_custom', {
                period: 10,
                high: 'high',
                low: 'low',
                close: 'close',
