...
describe('ATR (Average True Range) should return the correct value', () => {
    ...
    describe('Basic ATR calculation', () => {
        ...
        it('When dataset length is less than 2.', () => {
            const result = atr.calculate(new core_1.Dataset([data[0]]));
            expect(isNaN(result)).toBe(true);
        });
        it('When dataset length is 2.', () => {
            const result = atr.calculate(new core_1.Dataset(data.slice(0, 2)));
            expect(typeof result).toBe('number');
            expect(isNaN(result)).toBe(false);
            expect(result).toBeGreaterThan(0);
        });
        it('When dataset length is less than period + 1.', () => {
            const result = atr.calculate(new core_1.Dataset(data.slice(0, period)));
            expect(typeof result).toBe('number');
            expect(isNaN(result)).toBe(false);
            expect(result).toBeGreaterThan(0);
        });
    ...
    describe('True Range calculation', () => {
        ...
        it('True Range should be max of (High-Low, |High-PrevClose|, |Low-PrevClose|).', () => {
            const ds = new core_1.Dataset(data.slice(0, 5));
            const atr = new src_1.ATR('atr', {
                period: 14,
                high: 'high',
                low: 'low',
    ...
    describe('Custom parameters', () => {
        it('Should work with custom period.', () => {
            const ds = new core_1.Dataset(data);
            const atr = new src_1.ATR('atr_custom', {
                period: 10,
                high: 'high',
                low: 'low',
                close: 'close',
