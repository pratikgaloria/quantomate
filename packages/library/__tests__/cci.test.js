...
describe('CCI (Commodity Channel Index) should return the correct value', () => {
    ...
    describe('Basic CCI calculation', () => {
        ...
        it('When dataset length is less than period.', () => {
            const result = cci.calculate(new core_1.Dataset(data.slice(0, period - 1)));
            expect(isNaN(result)).toBe(true);
        });
        it('When dataset length is equal to period.', () => {
            const result = cci.calculate(new core_1.Dataset(data.slice(0, period)));
    ...
    describe('Custom parameters', () => {
        it('Should work with custom period.', () => {
            const ds = new core_1.Dataset(data);
            const cci = new src_1.CCI('cci_custom', {
                period: 14,
                high: 'high',
                low: 'low',
                close: 'close',
    ...
    describe('Unbounded scale', () => {
        it('CCI can exceed +100 or go below -100.', () => {
            ...
            const ds = new core_1.Dataset(volatileData);
            const cci = new src_1.CCI('cci', {
                period: 20,
                high: 'high',
                low: 'low',
