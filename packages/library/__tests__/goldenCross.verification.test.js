...
describe('GoldenCrossStrategy Verification', () => {
    it('should trigger buy signal when fast EMA crosses above slow SMA', () => {
        ...
        const dataset = new core_1.Dataset(prices.map(price => ({ close: price })));
        const strategy = new src_1.GoldenCrossStrategy('test-golden-cross', {
            fastPeriod: 9,
            slowPeriod: 20,
            source: 'close',
        });
    ...
    it('should calculate backtest correctly with simple data', () => {
        ...
        const dataset = new core_1.Dataset(prices.map(price => ({ close: price })));
        const strategy = new src_1.GoldenCrossStrategy('test-backtest', {
            fastPeriod: 5,
            slowPeriod: 10,
            source: 'close',
        });
