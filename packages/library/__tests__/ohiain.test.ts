...
import { OhiainStrategy } from '../src';
...
describe('OhiainStrategy', () => {
  it('should trigger buy signal on breakout with trend alignment', () => {
    ...
    const prices: { close: number; high: number; low: number; }[] = [];
    ...
    const dataset = new Dataset(prices);
    const strategy = new OhiainStrategy('ohiain-test', {
        periodEMA9: 9,
        periodEMA21: 21,
        periodEMA50: 50,
        periodATR: 14
    ...
    */
    ...
    for (let i = 50; i < dataset.length; i++) {
        if (dataset.at(i)?.getStrategy('ohiain-test')?.position?.value === 'entry') {
            ...
            const opts = strategyValue?.position.options as any;
  ...
  it('should respect stop loss', () => {
      ...
      const dataset = new Dataset(prices);
      const strategy = new OhiainStrategy('sl-test', {});
      dataset.prepare(strategy);
      ...
      if (entryPos?.value === 'entry') {
          ...
          expect((exitPos?.options as any).exitReason).toBe('stop-loss');
      }
  });
});
