...
import { PivotTrend, PIVOT_TREND_UP, PIVOT_TREND_DOWN, PIVOT_TREND_NEUTRAL } from '../src';
...
describe('PivotTrend indicator', () => {
  ...
  it('returns NaN when dataset length < 2', () => {
    ...
    const pt = new PivotTrend<typeof data[0]>('pt', { high: 'high', low: 'low', close: 'close' });
    expect(Number.isNaN(pt.calculate(ds))).toBe(true);
  });

  it('computes pivot R S and returns 1 when close > R', () => {
    const ds = new Dataset(data.slice(0, 2));
    const pt = new PivotTrend<typeof data[0]>('pt', { high: 'high', low: 'low', close: 'close' });
    ...
    expect(result).toBe(PIVOT_TREND_UP);
  });
  ...
  it('returns -1 when close < S', () => {
    ...
    const pt = new PivotTrend<typeof d[0]>('pt', { high: 'high', low: 'low', close: 'close' });
    const result = pt.calculate(ds);
    expect(result).toBe(PIVOT_TREND_DOWN);
  });
  ...
  it('returns 0 when close between S and R', () => {
    ...
    const pt = new PivotTrend<typeof d[0]>('pt', { high: 'high', low: 'low', close: 'close' });
    const result = pt.calculate(ds);
    expect(result).toBe(PIVOT_TREND_NEUTRAL);
  });

  it('spreads over dataset', () => {
    const ds = new Dataset(data);
    const pt = new PivotTrend<typeof data[0]>('pivotTrend', { high: 'high', low: 'low', close: 'close' });
    pt.spread(ds);
    ...
    expect(v1 === PIVOT_TREND_UP || v1 === PIVOT_TREND_DOWN || v1 === PIVOT_TREND_NEUTRAL).toBe(true);
  });
});
